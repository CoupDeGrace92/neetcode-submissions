...
func carFleet(target int, position []int, speed []int) int {
    ...
    for i:=len(stacks) - 1; i >=0; {
        total ++
        j:= i-1
        //TIMETOARRIVE: pos + x(speed) = target
        //This is rounded up - so (target-pos)/speed can be floor division
...
//Monotonicity - SORTING the position array, but we need to have it associated speeds stay sorted with it
//THEN - time to target

//My solutions space complexity does not feel like it will be optimal
